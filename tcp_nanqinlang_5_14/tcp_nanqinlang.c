/* Nanqinlang TCP congestion control
 *
 * Based on the kernel 5.14 BBR implementation, with the behavior changes
 * carried from the 4.14 nanqinlang variant.
 */
#include <linux/btf.h>
...
static struct tcp_congestion_ops tcp_nanqinlang_cong_ops __read_mostly = {
...
BTF_KFUNCS_START(tcp_nanqinlang_check_kfunc_ids)
#ifdef CONFIG_X86
...
#endif
#endif
BTF_KFUNCS_END(tcp_nanqinlang_check_kfunc_ids)

static const struct btf_kfunc_id_set tcp_nanqinlang_kfunc_set = {
	.owner = THIS_MODULE,
	.set = &tcp_nanqinlang_check_kfunc_ids,
};

static int __init nanqinlang_register(void)
{
	int ret;
	...
	ret = register_btf_kfunc_id_set(BPF_PROG_TYPE_STRUCT_OPS,
					&tcp_nanqinlang_kfunc_set);
	if (ret < 0)
		return ret;
	return tcp_register_congestion_control(&tcp_nanqinlang_cong_ops);
}

static void __exit nanqinlang_unregister(void)
{
	tcp_unregister_congestion_control(&tcp_nanqinlang_cong_ops);
}

module_init(nanqinlang_register);
module_exit(nanqinlang_unregister);
...
MODULE_DESCRIPTION("TCP nanqinlang (modified BBR)");